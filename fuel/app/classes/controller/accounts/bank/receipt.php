...
class Controller_Accounts_Bank_Receipt extends Controller_Authenticate
{
	public function action_index()
	{
		$data['bank_receipts'] = Model_Accounts_Bank_Receipt::find('all');
		$this->template->title = "Bank Deposits";
		$this->template->content = View::forge('accounts/bank/receipt/index', $data);
	}
	...
	public function action_view($id = null)
	{
		...
		if ( ! $data['bank_receipt'] = Model_Accounts_Bank_Receipt::find($id))
		{
		...
		$this->template->title = "Bank Deposits";
		$this->template->content = View::forge('accounts/bank/receipt/view', $data);
	}
	...
	public function action_create()
	{
		...
		{
			$val = Model_Accounts_Bank_Receipt::validate('create');

			if ($val->run())
			{
				$bank_receipt = Model_Accounts_Bank_Receipt::forge(array(
		...
		$this->template->title = "Bank Deposits";
		$this->template->content = View::forge('accounts/bank/receipt/create');
	}
	...
	public function action_edit($id = null)
	{
		...
		if ( ! $bank_receipt = Model_Accounts_Bank_Receipt::find($id))
		{
			Session::set_flash('error', 'Could not find bank receipt #'.$id);
			Response::redirect('bank/receipt');
		}

		$val = Model_Accounts_Bank_Receipt::validate('edit');
		...
		$this->template->title = "Bank Deposits";
		$this->template->content = View::forge('accounts/bank/receipt/edit');
	}

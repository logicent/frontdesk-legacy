...
class Controller_Bank_Account extends Controller_Authenticate
{
	...
	public function action_view($id = null)
	{
		...
		if ( ! $data['bank_account'] = Model_Bank_Account::find($id))
		{
		...
		$this->template->title = "Bank Accounts";
		$this->template->content = View::forge('bank/account/view', $data);
	...
	public function action_create()
	{
		...
		{
			$val = Model_Bank_Account::validate('create');

			if ($val->run())
			{
				$bank_account = Model_Bank_Account::forge(array(
		...
		$this->template->title = "Bank Accounts";
		$this->template->content = View::forge('bank/account/create');
	...
	public function action_edit($id = null)
	{
		...
		if ( ! $bank_account = Model_Bank_Account::find($id))
		{
			Session::set_flash('error', 'Could not find bank account #'.$id);
			Response::redirect('bank/account');
		}

		$val = Model_Bank_Account::validate('edit');
		...
		$this->template->title = "Bank Accounts";
		$this->template->content = View::forge('bank/account/edit');
	...
	public function action_delete($id = null)
	{
		...
		if ($bank_account = Model_Bank_Account::find($id))
		{
			$deposit = Model_Bank_Receipt::find('first', array('where' => array('bank_account_id' => $id)));
			if ($deposit)
